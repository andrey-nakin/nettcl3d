...
namespace proc {
	...
	class PopulatorWrapper : public Wrapper<&type::populator> {
		...
		static int create(Tcl_Interp * interp, int objc, Tcl_Obj * CONST objv[]) {
			...
			if ("grid3d" == methodName) {
				if (objc != 7)
					throw WrongNumArgs(interp, 1, objv, "xsize ysize zsize betaRng tauRng vRng");

				populator::Grid3d::Params params(
					phlib::TclUtils::getUInt(interp, objv[1]),
					phlib::TclUtils::getUInt(interp, objv[2]),
					phlib::TclUtils::getUInt(interp, objv[3]),
				...
				throw WrongArgValue(interp, "grid3d");
